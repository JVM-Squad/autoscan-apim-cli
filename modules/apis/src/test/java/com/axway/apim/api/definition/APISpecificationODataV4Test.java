...
public class APISpecificationODataV4Test {
	
	ObjectMapper mapper = new ObjectMapper();
	
	private static String TEST_PACKAGE = "/com/axway/apim/api/definition/odata/v4";
	...
	@Test
	public void testODataV4TrippingService() throws AppException, IOException {
		
		byte[] odataMetadata = getAPISpecificationContent(TEST_PACKAGE+"/ODataV4TrippinService.xml");
		byte[] odataOpenAPI3 = getAPISpecificationContent(TEST_PACKAGE+"/ODataV4TrippinService.json");
		APISpecification apiDefinition = APISpecificationFactory.getAPISpecification(odataMetadata, "tripping-odata-v4.xml$metadata", "OData-V4-Test-API");
		...
		Assert.assertEquals(apiDefinition.getDescription(), "The OData Service from tripping-odata-v4.xml$metadata");
	...
	@Test
	public void testODataV4SAPAccessSequenceService() throws AppException, IOException {
		
		byte[] odataMetadata = getAPISpecificationContent(TEST_PACKAGE+"/SAP-AccessSequence4PricingInSales.xml");
		byte[] odataOpenAPI3 = getAPISpecificationContent(TEST_PACKAGE+"/SAP-AccessSequence4PricingInSales.json");
		APISpecification apiDefinition = APISpecificationFactory.getAPISpecification(odataMetadata, "sap-access-sequence-odata-v4.xml$metadata", "OData-V4-Test-API");
		apiDefinition.configureBasepath("https://myhost.customer.com:8767/api/v1/myAPI", null);
		
		Assert.assertTrue(apiDefinition instanceof ODataV4Specification);
		Assert.assertEquals(apiDefinition.getDescription(), "The OData Service from sap-access-sequence-odata-v4.xml$metadata");
		if(apiDefinition.getApiSpecificationContent().length!=odataOpenAPI3.length) {
			System.out.print(new String(apiDefinition.getApiSpecificationContent(), StandardCharsets.UTF_8));
		}
		Assert.assertEquals(apiDefinition.getApiSpecificationContent(), odataOpenAPI3);
